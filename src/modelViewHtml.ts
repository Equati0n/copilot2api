...
<head>
  ...
  <style>
    ...
    @keyframes c2a-fade-in {
      from { opacity: 0; transform: translateY(4px); }
      to   { opacity: 1; transform: none; }
    }

    @keyframes c2a-slide-down {
    ...
    * {
    ...
    button.saved {
      ...
      animation: c2a-pulse 600ms var(--ease);
    }
    ...
    .view {
      padding: 10px;
      animation: c2a-fade-in var(--motion-slow) var(--ease);
    }
    ...
    .provider-preset-list {
      ...
      animation: c2a-slide-down var(--motion) var(--ease);
    ...
    .model-card {
      ...
      overflow: hidden;
      animation: c2a-fade-in var(--motion) var(--ease);
      transition: border-color var(--motion-fast) var(--ease),
                  box-shadow var(--motion) var(--ease);
    }
  ...
  <script nonce="${nonce}">
    ...
    function renderProviderList() {
      ...
      if (groups.length === 0) {
        ...
        empty.innerHTML = [
          '<svg class="empty-illustration" viewBox="0 0 96 96" fill="none" aria-hidden="true">',
          '  <rect x="14" y="22" width="68" height="52" rx="10" stroke="currentColor" stroke-width="2" opacity="0.55"/>',
          '  <path d="M14 36 H82" stroke="currentColor" stroke-width="2" opacity="0.45"/>',
          '  <circle cx="22" cy="29" r="1.6" fill="currentColor" opacity="0.7"/>',
          '  <circle cx="28" cy="29" r="1.6" fill="currentColor" opacity="0.7"/>',
          '  <circle cx="34" cy="29" r="1.6" fill="currentColor" opacity="0.7"/>',
          '  <path d="M28 50 H56" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" opacity="0.65"/>',
          '  <path d="M28 60 H46" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" opacity="0.45"/>',
          '  <circle cx="66" cy="58" r="9" stroke="currentColor" stroke-width="2"/>',
          '  <path d="M66 53 V63 M61 58 H71" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>',
          '</svg>',
          '<div class="empty-title">还没有供应商</div>',
          '<div class="empty-hint">点击右上角 <strong>新建供应商</strong>，填入 API Key 与 Base URL 后即可在 Copilot Chat 中选用。</div>'
        ].join('');
