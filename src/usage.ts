...
export function estimateMessageTokenCount(
  ...
  if ((message as { name?: string }).name) {
    tokens += estimateTextTokenCount((message as { name?: string }).name!);
  }
...
export function estimateTextTokenCount(value: string | undefined): number {
  if (!value) {
    return 0;
  }
  let ascii = 0;
...
function estimatePartTokenCount(part: unknown): number {
  ...
  if (part instanceof vscode.LanguageModelToolCallPart) {
    return (
      estimateTextTokenCount(part.name) +
  ...
  if (part instanceof vscode.LanguageModelToolResultPart) {
    return (
      estimateTextTokenCount(part.callId) +
      (part.content as readonly unknown[]).reduce(
        (sum: number, entry) => sum + estimatePartTokenCount(entry),
        0,
      )
    );
  }
