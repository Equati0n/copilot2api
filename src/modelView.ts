...
export class ModelManagerViewProvider implements vscode.WebviewViewProvider {
  static readonly viewType = 'copilot2api.modelsView';
  ...
  resolveWebviewView(webviewView: vscode.WebviewView): void {
    ...
    this.context.subscriptions.push(
      ...
      vscode.workspace.onDidChangeConfiguration((event) => {
        if (event.affectsConfiguration('copilot2api.models')) {
          this.postState();
        }
      }),
    );
  ...
  private async handleMessage(message: WebviewMessage): Promise<void> {
    switch (message.type) {
      ...
      case 'openSettings':
        await vscode.commands.executeCommand('workbench.action.openSettings', 'copilot2api');
        return;
