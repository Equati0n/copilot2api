...
const GLOBAL_KEY = 'copilot2api.apiKey';
const PROVIDER_PREFIX = 'copilot2api.apiKey.';
...
export class AuthManager {
  ...
  async promptForGlobalApiKey(): Promise<boolean> {
    ...
    vscode.window.showInformationMessage('Copilot2API default API key saved.');
    return true;
  }
  ...
  async promptForProviderApiKey(provider: string): Promise<boolean> {
    ...
    const apiKey = await vscode.window.showInputBox({
      title: `Copilot2API API Key: ${provider}`,
    ...
    if (!apiKey.trim()) {
      ...
      vscode.window.showInformationMessage(`Copilot2API API key for ${provider} cleared.`);
      return true;
    }

    await this.secrets.store(key, apiKey.trim());
    vscode.window.showInformationMessage(`Copilot2API API key for ${provider} saved.`);
    return true;
  }
