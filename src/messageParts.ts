...
export function isToolResultPart(value: unknown): value is {
  callId: string;
...
export function inputPartToText(part: unknown): string {
  ...
  if (part instanceof vscode.LanguageModelToolResultPart || isToolResultPart(part)) {
    return collectToolResultText((part as { content?: readonly unknown[] }).content);
  }
  if (part instanceof vscode.LanguageModelToolCallPart) {
    return '';
  }
  if (isPromptTsxPart(part)) {
    return unknownValueToText((part as { value: unknown }).value);
  }
...
export interface MessagePartsSummary {
  role: string;
  partCount: number;
  parts: Array<{ kind: string; textChars: number }>;
}
...
function isPromptTsxPart(value: unknown): value is { value: unknown } {
...
function unknownValueToText(value: unknown): string {
  ...
  const stringLeaves = collectStringLeaves(value, new WeakSet(), 0);
