...
export class Copilot2ApiProvider implements vscode.LanguageModelChatProvider {
  ...
  constructor(private readonly authManager: AuthManager, context: vscode.ExtensionContext) {
    context.subscriptions.push(
      ...
      vscode.workspace.onDidChangeConfiguration((event) => {
        if (event.affectsConfiguration('copilot2api')) {
          void this.refreshModelPicker();
        }
      }),
      context.secrets.onDidChange((event) => {
        if (event.key.startsWith('copilot2api.apiKey')) {
          void this.refreshModelPicker();
        }
      }),
    );
  ...
  async refreshModelPicker(): Promise<void> {
    ...
    try {
      await vscode.lm.selectChatModels({ vendor: 'copilot2api' });
    } catch (error) {
  ...
  async provideLanguageModelChatResponse(
    ...
    if (!model) {
      throw new Error(`Copilot2API model not found: ${modelInfo.id}`);
    }

    const apiKey = await this.authManager.getApiKey(model.provider);
    if (!apiKey) {
      throw new Error(`No API key configured for ${model.provider}. Run "Copilot2API: Set Provider API Key".`);
    }
    ...
    if (!response.ok) {
      ...
      if (shouldRetryDeepSeekWithoutThinking(model, prepared.body, errorText)) {
        ...
        if (response.ok) {
          if (!response.body) {
            throw new Error('Copilot2API API returned no response body.');
          }
      ...
      throw new Error(
        `Copilot2API API error: [${response.status}] ${response.statusText}${errorText ? `\n${errorText}` : ''}`,
      );
    }
